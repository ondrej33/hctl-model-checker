...
class Model:
    ...
    and also carries some needed info about the HCTL formula (state-variables).

    Attributes:
        name: Name of the model.
        bdd: A BDD manager.
        update_fns: Dictionary mapping BN variables to their update functions
        name_dict: Dictionary mapping short canonical variable names to original names
        names_props: Sequence with original names for props s0, s1 ... (in ASCII order)
        names_params: Sequence with original names for params p0, p1 ... (in ASCII order)
        names_vars: Sequence with original names for vars x, xx ... (in ASCII order)
    ...
    def __init__(self, name: str, bdd: BDD, names_props: List[str],
        ...
        # unit set initially includes whole colored state space, but can be later restricted
